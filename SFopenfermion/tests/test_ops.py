...
class TestQuadraticCoefficients(unittest.TestCase):
    ...
    def test_displacement_vector(self):
        H = QuadOperator('q0', -0.432/self.hbar) + QuadOperator('p0', 3.213/self.hbar)
        A, d = quadratic_coefficients(H)
        expected_A = np.zeros([2, 2])
        expected_d = np.array([3.213, 0.432])/self.hbar
    ...
    def test_Dgate_displacement(self):
        ...
        self.assertTrue(np.allclose(d, expected))
        self.assertTrue(np.allclose(a, np.sqrt(self.hbar/2)*t*(d[0]+d[1]*1j)))

    def test_Xgate_displacement(self):
        x = 0.1234
        H, t = xdisplacement(x, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.array([1, 0])/self.hbar
        self.assertTrue(np.allclose(d, expected))

    def test_Zgate_displacement(self):
        z = 0.654
        H, t = zdisplacement(z, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.array([0, 1])/self.hbar
        self.assertTrue(np.allclose(d, expected))
    ...
    def test_rotation_coeff(self):
        ...
        H, t = rotation(0.23)
        res, d = quadratic_coefficients(get_quad_operator(H, self.hbar))
        expected = -np.diag(np.array([1, 1]))/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([2])))

        # two modes
        H, t = rotation(0.23, mode=1)
        res, d = quadratic_coefficients(get_quad_operator(H, self.hbar))
        expected = np.zeros([4, 4])
        expected[1, 1] = -1/self.hbar
        expected[3, 3] = -1/self.hbar
        self.assertTrue(np.allclose(res, expected))
    ...
    def test_squeeze_coeff(self):
        ...
        H, t = squeezing(0.23)
        res, d = quadratic_coefficients(get_quad_operator(H, self.hbar))
        expected = -np.array([[0, 1], [1, 0]])/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([2])))

        # two modes
        H, t = squeezing(0.23, mode=1)
        res, d = quadratic_coefficients(get_quad_operator(H, self.hbar))
        expected = np.zeros([4, 4])
        expected[1, 3] = -1/self.hbar
        expected[3, 1] = -1/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([4])))

    def test_quadratic_phase_coeff(self):
        # one mode
        H, t = quadratic_phase(0.23, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.zeros([2, 2])
        expected[0, 0] = -1/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([2])))

        # two modes
        H, t = quadratic_phase(0.23, mode=1, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.zeros([4, 4])
        expected[1, 1] = -1/self.hbar
        self.assertTrue(np.allclose(res, expected))
    ...
    def test_beamsplitter_coeff(self):
        ...
        theta = 0.5423
        phi = 0.3242
        H, t = beamsplitter(theta, phi)
        res, d = quadratic_coefficients(get_quad_operator(H, hbar=self.hbar))
        expected = np.zeros([4, 4])
        expected[0, 3] = expected[3, 0] = -np.cos(np.pi-phi)/self.hbar
        expected[1, 2] = expected[2, 1] = np.cos(np.pi-phi)/self.hbar
        expected[0, 1] = expected[1, 0] = -np.sin(np.pi-phi)/self.hbar
        expected[2, 3] = expected[3, 2] = -np.sin(np.pi-phi)/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([4])))

    def test_two_mode_squeeze_coeff(self):
        H, t = two_mode_squeezing(0.23)
        res, d = quadratic_coefficients(get_quad_operator(H, self.hbar))
        expected = np.fliplr(np.diag([1]*4))/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([4])))

    def test_controlled_addition(self):
        H, t = controlled_addition(0.23, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.fliplr(np.diag([1, 0, 0, 1]))/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([4])))

    def test_controlled_phase(self):
        H, t = controlled_phase(0.23, hbar=self.hbar)
        res, d = quadratic_coefficients(H)
        expected = np.zeros([4, 4])
        expected[0, 1] = expected[1, 0] = -1/self.hbar
        self.assertTrue(np.allclose(res, expected))
        self.assertTrue(np.allclose(d, np.zeros([4])))
